...
contract LunchVenueTest is LunchVenue {
    ...
    // New Test Case verify stop vote function
    function stopContractTest() public {
        Assert.equal(stopContract(), false, "Contract has not stopped");
    }

    // Verify number of votes
    function voteCountTest() public {
        Assert.equal(numVotes, 3, "Number of votes should be 3");
    }

    // Verify number of friends
    function friendCountTest() public {
        Assert.equal(numFriends, 4, "Number of friends should be 4");
    }

    // Verify number of restaurants
    function restaurantCountTest() public {
        Assert.equal(numRestaurants, 2, "Number of restaurants should be 2");
    }

    function testAddRestaurant() public {
        Assert.equal(addRestaurant("GYG"), 3, "Should be equal to 3");
    }

    function testAddFriend() public {
        Assert.equal(addFriend(acc4, "Frank"), 5, "Should be equal to 5");
    }
